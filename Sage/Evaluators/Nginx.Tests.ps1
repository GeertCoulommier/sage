...
Describe 'Nginx Configuration' -Tag 'Evaluation' {
    ...
    Context 'Nginx Config Files' {
        It 'Config <ConfFile> should contain line <ContainsLine>' -ForEach $V.NginxConfFileTests {
            ...
            if ($ContainsTokens) {
                $ContentLines = [regex]::Split([string]$File.Content, '\r?\n')
                $MatchingLine = $ContentLines | Where-Object {
                    $Line = $_
                    foreach ($Token in $ContainsTokens) {
                        if ($Line -notmatch [regex]::Escape([string]$Token)) {
                            return $false
                        }
                    }
                    return $true
                }
                $MatchingLine | Should -Not -BeNullOrEmpty -Because "config file $ConfFile must contain all required tokens on one line"
            }
        }
    }
    ...
    Context 'Symlink Files' {
        It 'Symlink <SymlinkPath> should point to <ExpectedTarget>' -ForEach $V.SymlinkTests {
            ...
            $LinkDirectory = Split-Path -Path $Link.Path -Parent
            $ActualTarget = [string]$Link.LinkTarget
            if (-not [System.IO.Path]::IsPathRooted($ActualTarget)) {
                $ActualTarget = [System.IO.Path]::GetFullPath((Join-Path $LinkDirectory $ActualTarget))
            }

            $ExpectedTargetNormalized = [string]$ExpectedTarget
            if (-not [System.IO.Path]::IsPathRooted($ExpectedTargetNormalized)) {
                $ExpectedTargetNormalized = [System.IO.Path]::GetFullPath((Join-Path $LinkDirectory $ExpectedTargetNormalized))
            }

            $ActualTarget | Should -Be $ExpectedTargetNormalized
        }
    }
    ...
    Context 'Live Web Tests' {
        It 'URL <Url> should return content containing <ExpectedContent>' -ForEach $V.CurlTests {
            ...
            $CurlContent = if ($CurlResult.Content -is [System.Array]) {
                ($CurlResult.Content -join [Environment]::NewLine)
            }
            else {
                [string]$CurlResult.Content
            }
            $CurlContent | Should -Match ([regex]::Escape($ExpectedContent))
        }
    }
