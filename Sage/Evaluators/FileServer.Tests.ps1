...
Describe 'File Server Configuration' -Tag 'Evaluation' {
    ...
    BeforeAll {
        ...
        .PARAMETER Name
            The raw group name or NTFS IdentityReference to normalise.
        .OUTPUTS
        ...
        It 'Path for share <ShareName> should grant <ExpectedIdentity> <ExpectedRights>' -ForEach $V.NtfsTests {
            ...
            $MatchingAce = if ($AnyOfExpectedIdentities) {
                $PermEntries.Permissions | Where-Object {
                    $Ace = $_
                    $NormalizedAceIdentity = Resolve-AdGroupName $Ace.IdentityReference
                    $IdentityMatch = @($AnyOfExpectedIdentities) | Where-Object {
                        $Ace.IdentityReference -ilike "*$_*" -or
                        $NormalizedAceIdentity -ieq (Resolve-AdGroupName $_)
                    }
                ...
                $NormalizedExpected = Resolve-AdGroupName $ExpectedIdentity
                $PermEntries.Permissions | Where-Object {
                    $NormalizedActual = Resolve-AdGroupName $_.IdentityReference
                    (
                        $_.IdentityReference -ilike "*$ExpectedIdentity*" -or
                        $NormalizedActual -ieq $NormalizedExpected
                    ) -and
