...
Describe 'GPO Configuration' -Tag 'Evaluation' {
    ...
    Context 'GPO Links' {
        It 'GPO <GpoName> should <LinkExpectation>' -ForEach $V.GpoLinkTests {
            $MatchingGpo = $CollectedData.Gpos | Where-Object { $_.Name -eq $GpoName }
            $MatchingGpo | Should -Not -BeNullOrEmpty
    ...
    Context 'GPO Administrative Policies' {
        It 'GPO <GpoName> should have policy <PolicyName> in state <ExpectedState>' -ForEach $V.GpoPolicyTests {
            ...
            if ($AlternativePolicySets) {
                # Helper: returns $true when a single policy spec is satisfied by the GPO data.
                $PolicySpecPasses = {
                    param([hashtable] $Spec, $Gpo)
                    $Scope = switch ($Spec.ScopeType) {
                        'Computer' { $Gpo.ComputerScope }
                        'User' { $Gpo.UserScope }
                        default { $Gpo.ComputerScope + $Gpo.UserScope }
                    }
                    $Match = $Scope | Where-Object { $_.Type -eq 'Policy' -and $_.Settings.Name -eq $Spec.PolicyName }
                    if (-not $Match) { return $false }
                    $Selected = @($Match)[0]
                    if ($Spec.ExpectedState -and $Selected.Settings.State -ne $Spec.ExpectedState) { return $false }
                    if ($Spec.ExpectedPathRegex) {
                        $Evidence = @($Selected.Settings.Path, $Selected.Settings.RawXml) -join ' '
                        if ($Evidence -notmatch $Spec.ExpectedPathRegex) { return $false }
                    }
                    return $true
                }
                ...
                $PrimarySpec = @{
                    ...
                    ScopeType         = $ScopeType
                    ExpectedPathRegex = $ExpectedPathRegex
                }
                if (& $PolicySpecPasses $PrimarySpec $MatchingGpo) { return }

                # Try each alternative set (all specs in a set must pass).
                foreach ($AltSet in @($AlternativePolicySets)) {
                    $SetPassed = $true
                    foreach ($AltSpec in @($AltSet)) {
                        if (-not (& $PolicySpecPasses $AltSpec $MatchingGpo)) {
                            $SetPassed = $false
                            break
                        }
                    }
                ...
                if ($Optional) {
                    Set-ItResult -Skipped -Because "Optional policy '$PolicyName' and all alternative configurations are not present in GPO '$GpoName'."
                    return
                }
                $false | Should -BeTrue -Because "GPO '$GpoName' should have policy '$PolicyName' or one of its alternative configurations"
                return
            }

            # ── Standard path (no AlternativePolicySets) ─────────────────────
    ...
    Context 'GPO Drive Mappings' {
        It 'GPO <GpoName> should map drive <DriveLetter> to <DrivePath>' -ForEach $V.GpoDriveMapTests {
            ...
            $MatchingDrive = $AllSettings | Where-Object {
                $_.Type -eq 'DriveMap' -and ($_.Settings.Letter -replace ':', '') -eq ($DriveLetter -replace ':', '')
            }
            ...
            if ($ExpectedPathRegex) {
                $SelectedDrive.Settings.Path | Should -Match $ExpectedPathRegex
            }
            elseif ($DrivePath) {
