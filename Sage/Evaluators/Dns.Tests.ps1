...
Describe 'DNS Configuration' -Tag 'Evaluation' {
    ...
    Context 'A Records' {
        It 'A record <Name> should be <IP> in zone <Zone>' -ForEach ($V.ARecords | Where-Object { -not $_.AnyOfNames }) {
        ...
        It 'A record for <IP> in zone <Zone> should use one of the names <AnyOfNames>' -ForEach ($V.ARecords | Where-Object { $_.AnyOfNames }) {
            $MatchingRecord = $CollectedData.Records | Where-Object {
                $_.RecordType -eq 'A' -and
                $_.Value -eq $IP -and
                $_.ZoneName -eq $Zone -and
                $_.HostName -in @($AnyOfNames)
            }
            $MatchingRecord | Should -Not -BeNullOrEmpty
        }
    }
    ...
    Context 'PTR Records' {
        It 'PTR record <Name> should resolve to <ExpectedPtr> in zone <Zone>' -ForEach ($V.PtrRecords | Where-Object { -not $_.AnyOfExpectedPtr }) {
            $MatchingRecord = $CollectedData.Records | Where-Object {
            ...
            # Only verify the first hostname label — domain can differ between environments
            # e.g. ExpectedPtr 'dc1.proef.be' → only check that value starts with 'dc1.'
            $ExpectedHostLabel = ($ExpectedPtr -split '\.')[0]
            $ActualValue | Should -Match "(?i)^$([regex]::Escape($ExpectedHostLabel))(\.|$)"
        }

        It 'PTR record <Name> in zone <Zone> should resolve to one of <AnyOfExpectedPtr>' -ForEach ($V.PtrRecords | Where-Object { $_.AnyOfExpectedPtr }) {
            $MatchingRecord = $CollectedData.Records | Where-Object {
                $_.RecordType -eq 'PTR' -and
                $_.HostName -eq $Name -and
                $_.ZoneName -eq $Zone
            ...
            $MatchingRecord | Should -Not -BeNullOrEmpty
            $ActualValue = $MatchingRecord.Value -replace '\.$', ''
            $AllowedLabels = @($AnyOfExpectedPtr) | ForEach-Object { ($_ -split '\.')[0] }
            $MatchesAny = $AllowedLabels | Where-Object { $ActualValue -match "(?i)^$([regex]::Escape($_))(\.|$)" }
            $MatchesAny | Should -Not -BeNullOrEmpty -Because "PTR record should start with one of: $($AllowedLabels -join ', ')"
        }
    }
