...
function Invoke-Evaluation {
    ...
    # When KeyFilePath is provided, SSH key auth handles authentication without a
    # credential object — vault lookup is skipped entirely to avoid spurious
    # prompts when SageVault is not registered on the current machine.
    # When no KeyFilePath is given, credentials are fetched from the vault (or
    # prompted interactively as a fallback via -AllowPrompt).
    $CredentialCache = @{}
    $TargetCredentials = @{}
    if ($KeyFilePath) {
        $LogParams = @{
            Level    = 'Info'
            Category = 'Setup'
            Message  = 'KeyFilePath provided — skipping vault credential resolution. SSH key auth will be used for all targets.'
        }
        Write-Log @LogParams
    }
    else {
    ...
    if ($ThrottleLimit -gt 1) {
        ...
        $ParallelResults = $Students | ForEach-Object -ThrottleLimit $ThrottleLimit -Parallel {
            ...
            if ($Exam.KeepTempFiles) { $StuParams['KeepTempFiles'] = $true }
