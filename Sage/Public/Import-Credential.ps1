...
.NOTES
    One-time vault setup per machine (run once before your first Invoke-Evaluation):

      1. Install-Module Microsoft.PowerShell.SecretManagement -Scope CurrentUser
      2. Install-Module Microsoft.PowerShell.SecretStore    -Scope CurrentUser
      3. $cred = Get-Credential -UserName 'student'  # use the exam password
         Set-Credential -Name 'SageStudentPassword' -Credential $cred

    After setup, Import-Credential -Name 'SageStudentPassword' retrieves the stored
    credential silently.  Set-Credential auto-registers SageVault if it does not exist.

    Use a descriptive key name (e.g. 'SageStudentPassword') as the CredentialSecret
    value in exam.psd1 — never the literal password string.
.OUTPUTS
