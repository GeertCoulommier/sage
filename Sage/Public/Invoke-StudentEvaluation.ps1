...
function Invoke-StudentEvaluation {
    ...
    param(
        ...
        [Parameter()]                                                                      [switch] $KeepTempFiles
    )

    $ErrorActionPreference = 'Stop'

    # ── Apply exam-level defaults ─────────────────────────────────────────────
    if (-not $PSBoundParameters.ContainsKey('KeepTempFiles') -and $Exam.ContainsKey('KeepTempFiles')) {
        $KeepTempFiles = $Exam.KeepTempFiles
    }
    ...
    if (-not $StudentIp -or -not $StudentName -or -not $StudentEmail) {
        $LogParams = @{
            Level    = 'Warning'
            Category = 'Pipeline'
            Message  = "Row missing '$IpField', '$NameField', or '$EmailField' — skipped."
        }
        Write-Log @LogParams
    ...
    try {
        ...
        # ── Per-student collector result cache (keyed by "TargetName-CollectorName") ──
        # Shared VMs mean two categories can target the same collector on the same VM.
        # Cache the result so only the first category incurs the SSH round-trip.
        $CollectorCache = @{}
        ...
        foreach ($Cat in $Exam.Categories) {
            ...
            # Check collector cache before making the SSH round-trip
            ...
            if ($CollectorCache.ContainsKey($CollCacheKey)) {
                ...
                $LogParams = @{
                    Level    = 'Verbose'
                    Category = 'Collector'
                    Message  = "Collector cache hit for '$CollCacheKey' — skipping SSH round-trip."
                }
                Write-Log @LogParams
            }
            else {
        ...
        foreach ($S in $Sessions) {
            try {
                Close-RemoteSession -Session $S -KeepTempFiles:$KeepTempFiles
            }
            catch {
                $LogParams = @{
                    Level    = 'Warning'
