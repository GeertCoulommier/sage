...
function Format-GeneralConfigData {
    ...
    # Build a lookup from InterfaceAlias → NetAdapter (gateway + DNS live here)
    $AdapterLookup = @{}
    if ($Data.NetAdapters) {
        foreach ($Adapter in $Data.NetAdapters) {
            $Key = $Adapter.InterfaceAlias
            if ($Key -and -not $AdapterLookup.ContainsKey($Key)) {
                $AdapterLookup[$Key] = $Adapter
            }
        }
    }

    # IPAddresses key is IPAddresses in the collector; allow either casing
    $IpCollection = if ($Data.IPAddresses) { $Data.IPAddresses }
    elseif ($Data.IpAddresses) { $Data.IpAddresses }
    else { @() }

    foreach ($Ip in $IpCollection) {
        $Adapter = $AdapterLookup[$Ip.InterfaceAlias]
        $Gateway = if ($Adapter -and $Adapter.Gateway) { $Adapter.Gateway }    else { $Ip.Gateway }
        $DnsServers = if ($Adapter -and $Adapter.DnsServers) { $Adapter.DnsServers } else { $Ip.DnsServers }

        # Convert PrefixLength to dotted-decimal subnet mask
        $PrefixLength = [int]$Ip.PrefixLength
        $SubnetMask = if ($PrefixLength -ge 0 -and $PrefixLength -le 32) {
            $Bits = [uint32]([uint32]::MaxValue -shl (32 - $PrefixLength))
            "$([byte](($Bits -shr 24) -band 0xFF)).$([byte](($Bits -shr 16) -band 0xFF)).$([byte](($Bits -shr 8) -band 0xFF)).$([byte]($Bits -band 0xFF))"
        }
        else { '' }

        # Determine assignment type from PrefixOrigin
        $AssignType = switch ($Ip.PrefixOrigin) {
            'Dhcp' { 'DHCP' }
            'Manual' { 'Static' }
            'WellKnown' { 'Static' }
            'RouterAdvertisement' { 'DHCP' }
            default { if ($Ip.PrefixOrigin) { $Ip.PrefixOrigin } else { '' } }
        }

        $IpLine = "    IP     : $($Ip.IPAddress)/$PrefixLength ($SubnetMask)"
        if ($AssignType) { $IpLine += " [$AssignType]" }

        $null = $Sb.AppendLine("  Interface: $($Ip.InterfaceAlias)")
        $null = $Sb.AppendLine($IpLine)
        $null = $Sb.AppendLine("    Gateway: $(if ($Gateway) { $Gateway } else { '(none)' })")
        $null = $Sb.AppendLine("    DNS    : $(if ($DnsServers) { ($DnsServers -join ', ') } else { '(none)' })")
    }
    return $Sb.ToString()
}
...
function Format-AdData {
    ...
    if ($Data.Groups -and $Data.Groups.Count -gt 0) {
        # Split groups: user-created (in an OU) vs default/system (no OU in DN)
        $UserGroups = @($Data.Groups | Where-Object {
                (-not $_.DistinguishedName) -or ($_.DistinguishedName -match 'OU=')
            } | Sort-Object { $_.Name })
        $SystemGroups = @($Data.Groups | Where-Object {
                $_.DistinguishedName -and ($_.DistinguishedName -notmatch 'OU=')
            } | Sort-Object { $_.Name })

        # Compute column widths for an auto-sized table
        $AllGroups = @($UserGroups) + @($SystemGroups)
        $MaxNameLen = [Math]::Max(10, ($AllGroups | ForEach-Object { $_.Name.Length } | Measure-Object -Maximum).Maximum)
        $MaxScopeLen = [Math]::Max(8, ($AllGroups | ForEach-Object {
                    if ($_.GroupScope) { $_.GroupScope.Length } else { 0 }
                } | Measure-Object -Maximum).Maximum)

        $HeaderName = 'Name'.PadRight($MaxNameLen)
        $HeaderScope = 'Scope'.PadRight($MaxScopeLen)
        $Divider = "$('─' * $MaxNameLen)  $('─' * $MaxScopeLen)  ─────────"

        foreach ($Section in @(
                [PSCustomObject]@{ Label = 'User-created Groups'; List = $UserGroups }
                [PSCustomObject]@{ Label = 'Default/System Groups'; List = $SystemGroups }
            )) {
            if (-not $Section.List -or $Section.List.Count -eq 0) { continue }
            $null = $Sb.AppendLine("  ── $($Section.Label) ($($Section.List.Count)) ──")
            $null = $Sb.AppendLine("    $HeaderName  $HeaderScope  Members")
            $null = $Sb.AppendLine("    $Divider")
            foreach ($G in $Section.List) {
                $MemberCount = if ($G.Members) { $G.Members.Count } else { 0 }
                $Scope = if ($G.GroupScope) { $G.GroupScope } else { '' }
                $null = $Sb.AppendLine("    $($G.Name.PadRight($MaxNameLen))  $($Scope.PadRight($MaxScopeLen))  $MemberCount")
                if ($G.Members -and $G.Members.Count -gt 0) {
                    $Indent = ' ' * ($MaxNameLen + 4)
                    foreach ($M in $G.Members) {
                        $null = $Sb.AppendLine("$Indent- $M")
                    }
                }
            }
        }
...
function Format-FileServerData {
    ...
    # Well-known system/administrative share names
    $SystemShareNames = @('NETLOGON', 'SYSVOL', 'PRINT$', 'IPC$', 'ADMIN$')

    $IsSystemShare = {
        param([object] $Share)
        $N = $Share.Name
        $N -in $SystemShareNames -or $N.EndsWith('$')
    }

    $RenderShareGroup = {
        param([object[]] $ShareList, [string] $GroupLabel)
        if (-not $ShareList -or $ShareList.Count -eq 0) { return }
        $null = $Sb.AppendLine("  ══ $GroupLabel ══")
        foreach ($S in ($ShareList | Sort-Object { $_.Name })) {
            ...
            # NTFS permissions for this share (non-inherited only)
            $SharePerms = @($Data.Permissions | Where-Object { $_.ShareName -ieq $S.Name })
            if ($SharePerms.Count -gt 0) {
                $null = $Sb.AppendLine('    NTFS Permissions:')
                foreach ($P in $SharePerms) {
                    $RelLabel = if ($P.RelativePath -and $P.RelativePath -ne '.') { " \$($P.RelativePath)" } else { '' }
                    $null = $Sb.AppendLine("      $($S.Name)$RelLabel ($($P.Path)):")
                    if ($P.Permissions) {
    ...
    if ($Data.Shares -and $Data.Shares.Count -gt 0) {
        $UserShares = @($Data.Shares | Where-Object { -not (& $IsSystemShare $_) })
        $SystemShares = @($Data.Shares | Where-Object { (& $IsSystemShare $_) })
        & $RenderShareGroup $UserShares 'User Shares'
        & $RenderShareGroup $SystemShares 'System Shares'
    }
...
function Format-IisData {
    ...
    if ($Data.CurlResults -and $Data.CurlResults.Count -gt 0) {
        $null = $Sb.AppendLine('  ── Curl / HTTP Tests ──')
        foreach ($C in $Data.CurlResults) {
            if ($C.Status -ge 200 -and $C.Status -lt 400) {
                $Preview = ($C.Content -replace '\r?\n', ' ').Trim()
                if ($Preview.Length -gt 80) { $Preview = $Preview.Substring(0, 80) + '…' }
                $null = $Sb.AppendLine("    [$($C.Status)] $($C.Url)  →  $Preview")
            }
            else {
                $ErrMsg = if ($C.Error) { " ($($C.Error))" } else { '' }
                $null = $Sb.AppendLine("    [$($C.Status)] $($C.Url)$ErrMsg")
            }
        }
    }
    ...
    if ($Data.Websites -and $Data.Websites.Count -gt 0) {
        foreach ($Site in ($Data.Websites | Sort-Object { if ($_.Name -eq 'Default Web Site') { 'zzz' } else { $_.Name } })) {
            ...
            if ($Site.VirtualDirectories -and $Site.VirtualDirectories.Count -gt 0) {
                ...
                foreach ($V in $Site.VirtualDirectories) {
                    ...
                    $null = $Sb.AppendLine("        Dir Browsing: $($V.DirectoryBrowsing)")
                    if ($V.Authentication -and $V.Authentication.Count -gt 0) {
                        $NonDefault = @($V.Authentication | Where-Object { $_.Enabled })
                        if ($NonDefault.Count -gt 0) {
                            $null = $Sb.AppendLine("        Auth (enabled): $(@($NonDefault | ForEach-Object { $_.Method }) -join ', ')")
                        }
                        else {
                            $AllDisabled = @($V.Authentication | ForEach-Object { $_.Method }) -join ', '
                            $null = $Sb.AppendLine("        Auth (all disabled): $AllDisabled")
                        }
                    }
                }
            }
            ...
            if ($Site.IndexPath) {
                $null = $Sb.AppendLine("    Index file: $($Site.IndexPath)")
            }
            if ($Site.IndexContent) {
                $Preview = ($Site.IndexContent -replace '\r?\n', ' ').Trim()
                if ($Preview.Length -gt 120) { $Preview = $Preview.Substring(0, 120) + '…' }
                $null = $Sb.AppendLine("    Index content: $Preview")
            }
        }
    }
...
function Format-DockerData {
    ...
    if ($Data.CurlResults -and $Data.CurlResults.Count -gt 0) {
        $null = $Sb.AppendLine("  ── Curl Tests ($($Data.CurlResults.Count)) ──")
        foreach ($Cr in $Data.CurlResults) {
            $Status = if ($Cr.Success) { 'OK' } else { 'FAIL' }
            $null = $Sb.AppendLine("    [$Status] $($Cr.Url)")
            if ($Cr.Content) {
                $Preview = ($Cr.Content -split "`n" | Select-Object -First 3) -join ' '
                $null = $Sb.AppendLine("      $Preview")
            }
        }
    }

    if ($Data.FileContents -and $Data.FileContents.Count -gt 0) {
        $null = $Sb.AppendLine("  ── File Contents ($($Data.FileContents.Count)) ──")
        foreach ($Fc in $Data.FileContents) {
            $null = $Sb.AppendLine("    $($Fc.Path)")
            if ($Fc.Content) {
                $Lines = ($Fc.Content -split "`n") | ForEach-Object { $_.Trim() } | Where-Object { $_ }
                foreach ($Line in $Lines | Select-Object -First 10) {
                    $null = $Sb.AppendLine("      $Line")
                }
                if ($Lines.Count -gt 10) {
...
function Format-GeneralConfigDataMarkdown {
    ...
    if ($IpCollection.Count -gt 0) {
        # Build lookup from NetAdapters (gateway + DNS live here in real collector data)
        $AdapterLookup = @{}
        if ($Data.NetAdapters) {
            foreach ($Adapter in $Data.NetAdapters) {
                $Key = $Adapter.InterfaceAlias
        ...
        foreach ($Ip in $IpCollection) {
            ...
            $IpDisplay = "$($Ip.IPAddress)/$PrefixLength"
            if ($SubnetMask) { $IpDisplay += " ($SubnetMask)" }
            if ($AssignType) { $IpDisplay += " [$AssignType]" }
            ...
            $IpPairs = @(
                ...
                @('DNS Servers', $(if ($DnsServers) { $DnsServers -join ', ' } else { '' }))
            )
            $MaxKeyLen = 0
...
function Format-IisDataMarkdown {
    ...
    if ($Data.CurlResults -and $Data.CurlResults.Count -gt 0) {
        $null = $Sb.AppendLine('## HTTP / Curl Tests')
        $null = $Sb.AppendLine()
        $null = $Sb.AppendLine($Fence + 'text')
        foreach ($C in $Data.CurlResults) {
            if ($C.Status -ge 200 -and $C.Status -lt 400) {
                ...
                $null = $Sb.AppendLine("[$($C.Status)] $($C.Url)")
                $null = $Sb.AppendLine("       $Preview")
            }
            else {
                $ErrMsg = if ($C.Error) { "  error: $($C.Error)" } else { '' }
                $null = $Sb.AppendLine("[$($C.Status)] $($C.Url)$ErrMsg")
            }
        }
        $null = $Sb.AppendLine($Fence)
        ...
        foreach ($Site in ($Data.Websites | Sort-Object { if ($_.Name -eq 'Default Web Site') { 'zzz' } else { $_.Name } })) {
            ...
            if ($Site.VirtualDirectories -and $Site.VirtualDirectories.Count -gt 0) {
                ...
                foreach ($V in $Site.VirtualDirectories) {
                    ...
                    $null = $Sb.AppendLine("$(' ' * ($MaxVd + 4))Dir Browsing: $($V.DirectoryBrowsing)")
                    if ($V.Authentication -and $V.Authentication.Count -gt 0) {
                        $NonDefault = @($V.Authentication | Where-Object { $_.Enabled })
                        if ($NonDefault.Count -gt 0) {
                            $AuthStr = @($NonDefault | ForEach-Object { $_.Method }) -join ', '
                            $null = $Sb.AppendLine("$(' ' * ($MaxVd + 4))Auth (enabled): $AuthStr")
                        }
                        else {
                            $AllMethods = @($V.Authentication | ForEach-Object { $_.Method }) -join ', '
                            $null = $Sb.AppendLine("$(' ' * ($MaxVd + 4))Auth (all disabled): $AllMethods")
                        }
                    }
                }
                $null = $Sb.AppendLine($Fence)
            ...
            if ($Site.IndexPath) {
                $null = $Sb.AppendLine('### Index File')
                $null = $Sb.AppendLine()
                $null = $Sb.AppendLine($Fence + 'text')
                $null = $Sb.AppendLine($Site.IndexPath)
                $null = $Sb.AppendLine($Fence)
                $null = $Sb.AppendLine()
            }

            if ($Site.IndexContent) {
                $null = $Sb.AppendLine('### Index Content')
                $null = $Sb.AppendLine()
                $null = $Sb.AppendLine($Fence + 'html')
                $null = $Sb.AppendLine($Site.IndexContent.TrimEnd())
                $null = $Sb.AppendLine($Fence)
                $null = $Sb.AppendLine()
            }
        }
...
function Format-DockerDataMarkdown {
    ...
    if ($Data.CurlResults -and $Data.CurlResults.Count -gt 0) {
        ...
        foreach ($Cr in $Data.CurlResults) {
            ...
            if ($Cr.Content) {
                $null = $Sb.AppendLine($Fence + 'html')
                $ContentLines = ($Cr.Content -split "`n") | ForEach-Object { $_.TrimEnd() }
                foreach ($Line in ($ContentLines | Select-Object -First 20)) {
                    $null = $Sb.AppendLine($Line)
                }
                if ($ContentLines.Count -gt 20) {
                    $null = $Sb.AppendLine("<!-- ... ($($ContentLines.Count - 20) more lines) -->")
                }
                $null = $Sb.AppendLine($Fence)
                $null = $Sb.AppendLine()
            }
    ...
    if ($Data.FileContents -and $Data.FileContents.Count -gt 0) {
        foreach ($Fc in $Data.FileContents) {
            $null = $Sb.AppendLine("## File: $($Fc.Path)")
            $null = $Sb.AppendLine()
            if ($Fc.Content) {
                $null = $Sb.AppendLine($Fence + 'html')
                $ContentLines = ($Fc.Content -split "`n") | ForEach-Object { $_.TrimEnd() }
                foreach ($Line in ($ContentLines | Select-Object -First 30)) {
                    $null = $Sb.AppendLine($Line)
                }
                if ($ContentLines.Count -gt 30) {
                    $null = $Sb.AppendLine("<!-- ... ($($ContentLines.Count - 30) more lines) -->")
                }
                $null = $Sb.AppendLine($Fence)
            } else {
                $null = $Sb.AppendLine('_File is empty or not found._')
            }
            $null = $Sb.AppendLine()
        }
    }
