...
function Invoke-LocalEvaluation {
    ...
    # ── Re-order CategoryScores to match exam definition order ─────────────────
    if ($Result.Summary -and $Result.Summary.CategoryScores) {
        $ExamCategoryOrder = @{}
        $OrderIdx = 0
        foreach ($Cat in $FilteredExam.Categories) {
            if (-not $ExamCategoryOrder.ContainsKey($Cat.Name)) {
                $ExamCategoryOrder[$Cat.Name] = $OrderIdx
                $OrderIdx++
            }
        }
        $Result.Summary.CategoryScores = @(
            $Result.Summary.CategoryScores | Sort-Object {
                if ($ExamCategoryOrder.ContainsKey($_.Category)) {
                    $ExamCategoryOrder[$_.Category]
                }
                else { 9999 }
            }
        )
