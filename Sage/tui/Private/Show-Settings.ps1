...
.DESCRIPTION
    ...
    editor, theme selector, result cleaner, and SSH key removal.
...
function Show-Settings {
    ...
    $VanillaConfigFile  = Join-Path $TuiPath 'tui-config.psd1'
    $PersonalConfigFile = [System.IO.Path]::GetFullPath((Join-Path $TuiPath '..' 'data' 'config' 'tui-config-personal.psd1'))
    ...
    $BuildItems = {
        ...
        $Items.Add(@{ Label = 'Edit tui-config-personal.psd1';  Value = '';         Type = 'action'; Key = 'EditConfig' })
        ...
        $Items.Add(@{ Label = 'Remove tui-config-personal.psd1'; Value = '';         Type = 'action'; Key = 'RemovePersonalConfig' })
        $Items.Add(@{ Label = 'Remove SSH key pair';             Value = '';         Type = 'action'; Key = 'RemoveSshKeys' })
        ...
        switch ($Key.Key.ToString()) {
            ...
            'Enter' {
                ...
                switch ($Item.Key) {
                    ...
                    'EditConfig' {
                        if (-not (Test-Path $PersonalConfigFile) -and (Test-Path $VanillaConfigFile)) {
                            $PersonalCfgDir = Split-Path $PersonalConfigFile -Parent
                            if (-not (Test-Path $PersonalCfgDir)) {
                                New-Item -ItemType Directory -Force -Path $PersonalCfgDir | Out-Null
                            }
                            Copy-Item -Path $VanillaConfigFile -Destination $PersonalConfigFile -Force
                        }
                        if (Test-Path $PersonalConfigFile) {
                            $Editor = if ($env:EDITOR) { $env:EDITOR } else { 'nano' }
                            & $Editor $PersonalConfigFile
                        }
                        else {
                            Write-Host '  tui-config-personal.psd1 not found.' -ForegroundColor Yellow
                        }
                    }
                    'ClearResults' {
                        $EffDir = $OutputDir
                    ...
                    'RemovePersonalConfig' {
                        if (Test-Path $PersonalConfigFile) {
                            $Confirm = Read-Host '  Are you sure? This deletes tui-config-personal.psd1. (y/N)'
                            if ($Confirm.Trim().ToUpper() -eq 'Y') {
                                Remove-Item -Path $PersonalConfigFile -Force
                                Write-Host '  tui-config-personal.psd1 removed.' -ForegroundColor Green
                            }
                        }
                        else {
                            Write-Host '  tui-config-personal.psd1 does not exist.' -ForegroundColor Yellow
                        }
                    }
                    'RemoveSshKeys' {
                        $KeysDir    = [System.IO.Path]::GetFullPath((Join-Path $TuiPath 'keys'))
                        $PrivateKey = Join-Path $KeysDir 'id_sage'
                        $PublicKey  = Join-Path $KeysDir 'id_sage.pub'
                        if ((Test-Path $PrivateKey) -or (Test-Path $PublicKey)) {
                            $Confirm = Read-Host '  Are you sure? This deletes the SAGE SSH key pair. (y/N)'
                            if ($Confirm.Trim().ToUpper() -eq 'Y') {
                                if (Test-Path $PrivateKey) { Remove-Item -Path $PrivateKey -Force }
                                if (Test-Path $PublicKey)  { Remove-Item -Path $PublicKey  -Force }
                                Write-Host '  SSH key pair removed.' -ForegroundColor Green
                            }
                        }
                        else {
                            Write-Host '  No SAGE SSH keys found.' -ForegroundColor Yellow
                        }
                    }
