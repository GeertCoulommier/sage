...
<#
.SYNOPSIS
    Saves the list of preferred fallback targets to exam.psd1.
.DESCRIPTION
    Updates the Remembered.PreferFallbackTargets array in the TUI config.
.PARAMETER ConfigPath
    Absolute path to the exam.psd1 file.
.PARAMETER Targets
    Array of target names to mark as preferring fallback.
.OUTPUTS
    [void]
.EXAMPLE
    Save-PreferFallbackTargetsInExamConfig -ConfigPath '/path/tui/exam.psd1' -Targets @('Linux', 'DC1')
#>
function Save-PreferFallbackTargetsInExamConfig {
    ...
    $Result = [System.Collections.Generic.List[string]]::new()
    $InRemembered = $false
    $PreferInserted = $false
    $Depth = 0

    foreach ($Line in $Lines) {
        # Detect Remembered block
        ...
        if ($InRemembered) {
            ...
            # Replace or insert PreferFallbackTargets
            if ($Line -match '^\s*PreferFallbackTargets\s*=') {
                $TargetArray = ($Targets | ForEach-Object { "'$_'" }) -join ', '
                $Result.Add("        PreferFallbackTargets = @($TargetArray)")
                $PreferInserted = $true
                continue
            }

            # If we're closing the Remembered block and haven't inserted yet, do it now
            if ($Depth -le 0) {
                if (-not $PreferInserted) {
                    $TargetArray = ($Targets | ForEach-Object { "'$_'" }) -join ', '
                    $Result.Add("        PreferFallbackTargets = @($TargetArray)")
                }
                $InRemembered = $false
    ...
    [System.IO.File]::WriteAllLines($ConfigPath, $Result, [System.Text.Encoding]::UTF8)
    Write-Verbose "Saved preferred fallback targets to '$ConfigPath'."
}
