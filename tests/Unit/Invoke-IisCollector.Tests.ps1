...
Describe 'Invoke-IisCollector' -Tag 'Unit' {
    ...
    Context 'PS 7 direct IISAdministration available' {
        BeforeEach {
            ...
            $MockConfig | Add-Member -MemberType ScriptMethod -Name 'GetSection' -Value {
                param([string]$SectionPath, [string]$LocationPath)
            ...
            $MockMgr | Add-Member -MemberType ScriptMethod -Name 'GetApplicationHostConfiguration' -Value {
                $Config = [PSCustomObject]@{}
                $Config | Add-Member -MemberType ScriptMethod -Name 'GetSection' -Value {
                    param([string]$SectionPath, [string]$LocationPath)
                    return @{ enabled = $false }
                }
                return $Config
            }
            $MockMgr | Add-Member -MemberType ScriptMethod -Name 'GetWebConfiguration' -Value {
                param([string]$SiteName, [string]$VirtualPath)
                $Cfg = [PSCustomObject]@{}
                $Cfg | Add-Member -MemberType ScriptMethod -Name 'GetSection' -Value {
    ...
    Context 'ConvertTo-BindingUri logic' {
        ...
        It 'Converts wildcard binding with host header' {
            ...
            $MockPool = [PSCustomObject]@{ Name = 'Pool1'; State = 'Started'; ManagedPipelineMode = 'Integrated'; ManagedRuntimeVersion = 'v4.0' }
            $ThisMgr = [PSCustomObject]@{ Sites = @($MockSite); ApplicationPools = @($MockPool) }
            $ThisMgr | Add-Member -MemberType ScriptMethod -Name 'GetApplicationHostConfiguration' -Value {
                $C = [PSCustomObject]@{}
                $C | Add-Member -MemberType ScriptMethod -Name 'GetSection' -Value { return @{ enabled = $false } }
                return $C
            }
            $ThisMgr | Add-Member -MemberType ScriptMethod -Name 'GetWebConfiguration' -Value {
                param([string]$SiteName, [string]$VirtualPath)
                $Cfg = [PSCustomObject]@{}
                $Cfg | Add-Member -MemberType ScriptMethod -Name 'GetSection' -Value {
                    param([string]$SectionPath)
            ...
            Mock Get-IISServerManager { $ThisMgr }
            $Result = & $Sut -Variables @{}
    ...
    Context 'Result structure' {
        BeforeEach {
            ...
            Mock Get-IISServerManager {
                ...
                $Mgr | Add-Member -MemberType ScriptMethod -Name 'GetApplicationHostConfiguration' -Value {
                    $Config = [PSCustomObject]@{}
                    $Config | Add-Member -MemberType ScriptMethod -Name 'GetSection' -Value {
                        param([string]$SectionPath, [string]$LocationPath)
                        return @{ enabled = $false }
                ...
                $Mgr | Add-Member -MemberType ScriptMethod -Name 'GetWebConfiguration' -Value {
                    param([string]$SiteName, [string]$VirtualPath)
                    $Cfg = [PSCustomObject]@{}
                    $Cfg | Add-Member -MemberType ScriptMethod -Name 'GetSection' -Value {
                        param([string]$SectionPath)
    ...
    # ── DirectoryBrowsing: OR of applicationHost + web.config sources ─────────────
    Context 'DirectoryBrowsing reads from both config sources' {
        BeforeEach {
            Mock Import-Module { } -ParameterFilter { $Name -eq 'IISAdministration' }
            Mock Test-Path { $false }
        }

        It 'Reports true when GetWebConfiguration (web.config) returns enabled=true' {
            $Site = [PSCustomObject]@{
                Name = 'S'; State = 'Started'
                Bindings = @([PSCustomObject]@{ Protocol = 'http'; BindingInformation = '*:80:' })
                Applications = @([PSCustomObject]@{
                        Path = '/'; ApplicationPoolName = 'P'
                        VirtualDirectories = @([PSCustomObject]@{ Path = '/'; PhysicalPath = 'C:\web' })
                    })
            }
            $Pool = [PSCustomObject]@{ Name = 'P'; State = 'Started'; ManagedPipelineMode = 'Integrated'; ManagedRuntimeVersion = 'v4.0' }
            $M = [PSCustomObject]@{ Sites = @($Site); ApplicationPools = @($Pool) }
            $M | Add-Member -MemberType ScriptMethod -Name 'GetApplicationHostConfiguration' -Value {
                $C = [PSCustomObject]@{}
                $C | Add-Member -MemberType ScriptMethod -Name 'GetSection' -Value { return @{ enabled = $false } }
                return $C
            }
            $M | Add-Member -MemberType ScriptMethod -Name 'GetWebConfiguration' -Value {
                param([string]$SiteName, [string]$VirtualPath)
                $Cfg = [PSCustomObject]@{}
                $Cfg | Add-Member -MemberType ScriptMethod -Name 'GetSection' -Value {
                    param([string]$SectionPath)
            ...
            Mock Get-IISServerManager { $M }
            $Result = & $Sut -Variables @{}
            $Result.Data.Websites[0].VirtualDirectories[0].DirectoryBrowsing | Should -BeTrue
        }

        It 'Reports true when applicationHost GetSection returns enabled=true' {
            $Site = [PSCustomObject]@{
                Name = 'S'; State = 'Started'
                Bindings = @([PSCustomObject]@{ Protocol = 'http'; BindingInformation = '*:80:' })
                Applications = @([PSCustomObject]@{
            ...
            $M | Add-Member -MemberType ScriptMethod -Name 'GetApplicationHostConfiguration' -Value {
                $C = [PSCustomObject]@{}
                $C | Add-Member -MemberType ScriptMethod -Name 'GetSection' -Value { return @{ enabled = $true } }
                return $C
            }
            $M | Add-Member -MemberType ScriptMethod -Name 'GetWebConfiguration' -Value {
                param([string]$SiteName, [string]$VirtualPath)
        ...
        It 'Reports false when both sources return enabled=false' {
            $Site = [PSCustomObject]@{
                Name = 'S'; State = 'Started'
                Bindings = @([PSCustomObject]@{ Protocol = 'http'; BindingInformation = '*:80:' })
                Applications = @([PSCustomObject]@{
            ...
            Mock Get-IISServerManager { $M }
            $Result = & $Sut -Variables @{}
            $Result.Data.Websites[0].VirtualDirectories[0].DirectoryBrowsing | Should -BeFalse
        }
    }
}
