...
Describe 'Invoke-StudentEvaluation' -Tag 'Unit' {
    ...
    # ── Collector result cache (P5) ────────────────────────────────────────────
    Context 'Collector result cache' -Tag 'Performance' {

        It 'Calls Invoke-RemoteCollector only once when two categories share the same collector on the same target' {
            $Exam = New-FakeExam
            $Exam.Categories = @(
                @{
            ...
            Invoke-Stu -Overrides @{ Exam = $Exam }
            Should -Invoke Invoke-RemoteCollector -Times 1 -Exactly -Because 'Second category shares the same collector on the same target — cache hit'
        }

        It 'Calls Invoke-RemoteCollector once per unique collector when collectors differ' {
            $Exam = New-FakeExam
            $Exam.Categories = @(
                @{
                    Name       = 'DNS'
            ...
            Invoke-Stu -Overrides @{ Exam = $Exam }
            Should -Invoke Invoke-RemoteCollector -Times 2 -Exactly -Because 'Different collectors on the same target each need one SSH call'
        }

        It 'Logs a Verbose message on a collector cache hit' {
            $Exam = New-FakeExam
            $Exam.Categories = @(
                @{
                    Name       = 'Active Directory DC1'
            ...
            Invoke-Stu -Overrides @{ Exam = $Exam }
            Should -Invoke Write-Log -ParameterFilter {
                $Level -eq 'Verbose' -and $Message -match 'cache hit'
            } -Because 'Cache hit should be logged at Verbose level'
        }
    }

    # ── KeepTempFiles pass-through (Step 0 verification) ────────────────────
    Context 'KeepTempFiles parameter' {

        It 'Passes -KeepTempFiles to Close-RemoteSession when set' {
            Invoke-Stu -Overrides @{ KeepTempFiles = $true }
            Should -Invoke Close-RemoteSession -ParameterFilter { $KeepTempFiles -eq $true } `
                -Because 'KeepTempFiles parameter should flow through to session cleanup'
        }

        It 'Does NOT pass -KeepTempFiles to Close-RemoteSession when not set' {
            Invoke-Stu
            Should -Invoke Close-RemoteSession -ParameterFilter { -not $KeepTempFiles } `
                -Because 'Default behavior should NOT set KeepTempFiles switch'
        }

        It 'Respects Exam.KeepTempFiles when provided' {
            $Exam = New-FakeExam
            $Exam.KeepTempFiles = $true
            Invoke-Stu -Overrides @{ Exam = $Exam }
            Should -Invoke Close-RemoteSession -ParameterFilter { $KeepTempFiles -eq $true } `
                -Because 'Exam-level KeepTempFiles setting should be honored'
        }
    }
}
