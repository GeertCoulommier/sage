...
Describe 'New-RemoteSession' -Tag 'Unit' {
    ...
    Context 'Credential accepted without KeyFilePath' {
        It 'Does not throw when -Credential is supplied without -KeyFilePath' {
            ...
            $Cred = [System.Management.Automation.PSCredential]::new(
                'user',
                (ConvertTo-SecureString $TestPw -AsPlainText -Force)
            )
    ...
    Context 'SSH keepalive defaults' {
        It 'Passes ServerAliveInterval, ServerAliveCountMax, and LogLevel to New-PSSession -Options' {
            New-RemoteSession @script:BaseParams
            Should -Invoke New-PSSession -Times 1 -ParameterFilter {
                $Options -and
                $Options['ServerAliveInterval'] -eq '15' -and
                $Options['ServerAliveCountMax'] -eq '3' -and
                $Options['LogLevel'] -eq 'ERROR'
            }
        }
    ...
    Context 'Password auth via SSH_ASKPASS (no KeyFilePath)' {
        ...
        It 'Succeeds when -Credential is supplied without -KeyFilePath' {
            $TestPw = 'testpw'
            $Cred = [System.Management.Automation.PSCredential]::new(
                'student',
                (ConvertTo-SecureString $TestPw -AsPlainText -Force)
            )
            $Params = $script:BaseParams + @{ Credential = $Cred }
            { New-RemoteSession @Params -ErrorAction Stop } | Should -Not -Throw
        }

        It 'Succeeds when -Password (SecureString) is supplied without -KeyFilePath' {
            $TestPw = 'testpw'
            $SecurePw = ConvertTo-SecureString $TestPw -AsPlainText -Force
            $Params = @{
                HostName   = '10.0.0.1'
                Port       = 20022
                UserName   = 'student'
            ...
            { New-RemoteSession @Params -Password $SecurePw -ErrorAction Stop } | Should -Not -Throw
        }

        It 'Still calls New-PSSession once when password auth is configured' {
            $TestPw = 'testpw'
            $Cred = [System.Management.Automation.PSCredential]::new(
                'student',
                (ConvertTo-SecureString $TestPw -AsPlainText -Force)
            ...
            $Params = $script:BaseParams + @{ Credential = $Cred }
            New-RemoteSession @Params
            Should -Invoke New-PSSession -Times 1
        }

        It 'Does NOT configure SSH_ASKPASS when -KeyFilePath is also supplied' {
            $TestPw = 'testpw'
            $Cred = [System.Management.Automation.PSCredential]::new(
                'student',
                (ConvertTo-SecureString $TestPw -AsPlainText -Force)
            )
            $Params = $script:BaseParams + @{
                Credential  = $Cred
                KeyFilePath = '/home/vscode/.ssh/id_ed25519'
            }
            New-RemoteSession @Params
            # Set-Item is the call that creates the SAGE_SSH_PW_* env var;
            # it should NOT be invoked when a key file is provided.
