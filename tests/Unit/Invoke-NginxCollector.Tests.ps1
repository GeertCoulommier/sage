...
Describe 'Invoke-NginxCollector' -Tag 'Unit' {
    ...
    Context 'Curl test result normalization' {
        BeforeEach {
            Mock Get-Command { [PSCustomObject]@{ Name = 'nginx' } } -ParameterFilter { $Name -eq 'nginx' }
            Mock systemctl { 'inactive' }
            Mock Test-Path { $false }
            Mock Get-ChildItem { @() }
            Mock curl { @('<html>', '<body>nginx.proef.be</body>', '</html>') }
        }

        It 'Stores curl response content as a single string' {
            $Vars = @{
                CurlTests = @(
                    @{
            ...
            $Result.Data.CurlResults[0].Content | Should -Match 'nginx\.proef\.be'
        }
    }
}
