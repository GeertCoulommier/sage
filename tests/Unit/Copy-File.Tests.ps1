...
Describe 'Copy-File' -Tag 'Unit' {
    ...
    Context '-SkipDirectoryCheck (P12)' -Tag 'Performance' {
        BeforeAll {
            $script:TmpFileSkip = Join-Path ([System.IO.Path]::GetTempPath()) "sage-copyfile-skip-$([guid]::NewGuid().ToString('N').Substring(0,8)).ps1"
            'content' | Set-Content -Path $script:TmpFileSkip
        }
        AfterAll {
            Remove-Item $script:TmpFileSkip -Force -ErrorAction SilentlyContinue
        }

        It 'Does not call Invoke-Command when -SkipDirectoryCheck is set' {
            Copy-File -Session $script:FakeSession -LocalPath $script:TmpFileSkip -RemotePath '/tmp/sage-collectors/test.ps1' -SkipDirectoryCheck
            Should -Invoke Invoke-Command -Times 0
        }

        It 'Still calls Copy-Item when -SkipDirectoryCheck is set' {
            Copy-File -Session $script:FakeSession -LocalPath $script:TmpFileSkip -RemotePath '/tmp/sage-collectors/test.ps1' -SkipDirectoryCheck
            Should -Invoke Copy-Item -Times 1
        }
    }
