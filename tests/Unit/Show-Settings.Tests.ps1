...
Describe 'Show-Settings' -Tag 'Unit' -Skip:$script:SkipInteractiveUiTests {
    ...
    Context 'Navigation' {
        ...
        It 'Returns Back when Back action is selected with Enter (cursor 6)' {
            $script:CallCount = 0
            Mock Invoke-ReadKey {
                $script:CallCount++
                if ($script:CallCount -le 6) { return New-DownKey }
                return New-EnterKey
            }
            Mock Write-Host { }
    ...
    Context 'Clear results' {
        ...
        It 'Deletes timestamped directories on confirmation' {
            ...
            Mock Invoke-ReadKey {
                $script:NavCount++
                if ($script:NavCount -le 3) { return New-DownKey }   # navigate to 'Clear all previous results' (index 3)
                if ($script:NavCount -eq 4) { return New-EnterKey }  # select 'Clear all previous results'
