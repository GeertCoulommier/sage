...
Describe 'Format-CollectorData' -Tag 'Unit' {
    ...
    Context 'Ad formatter' {
        It 'Renders domain info, computers, OUs, users, and groups' {
            ...
            $Result | Should -Match 'IT Staff'
            $Result | Should -Match 'Global'
            $Result | Should -Match '- jdoe'
        }
        ...
        It 'Handles Group without GroupScope and no Members' {
            ...
            $Result | Should -Match 'EmptyGroup.*0'
        }
    ...
    Context 'Docker formatter' {
        ...
        It 'Renders curl results in Docker plain-text output' {
            $Data = @{
                Images       = @()
                Containers   = @()
                Dockerfile   = @()
            ...
            $Result | Should -Match 'Curl Tests \(1\)'
            $Result | Should -Match 'http://192.168.1.2:8081'
        }

        It 'Renders file contents in Docker plain-text output' {
            $Data = @{
                Images       = @()
                Containers   = @()
                Dockerfile   = @()
                ...
                FileContents = @(@{
                    ...
                    Content = '<html>volumetest</html>'
                })
            }
            $Result = Format-CollectorData -CollectorResult @{
                Available = $true; Data = $Data; Errors = @()
            } -CollectorName 'Docker' -CategoryName 'Docker' -TargetName 'LNX1'
            $Result | Should -Match 'File Contents \(1\)'
            $Result | Should -Match '/home/student/volumetest/index.html'
        }
    }
...
Describe 'Format-CollectorDataMarkdown' -Tag 'Unit' {
    ...
    Context 'GeneralConfig Markdown formatter' {
        It 'Renders hostname, subnet mask, assignment type, gateway, and DNS servers' {
            $Data = @{
                Hostname    = 'DC1'
                IPAddresses = @(
                    @{
            ...
            $Result | Should -Match 'Hostname:.*DC1'
            $Result | Should -Match '### Ethernet'
            $Result | Should -Match 'IP Address:.*192\.168\.1\.10/24.*255\.255\.255\.0.*\[Static\]'
            $Result | Should -Match 'Gateway:.*192\.168\.1\.1'
            $Result | Should -Match '192\.168\.1\.1, 8\.8\.8\.8'
        }
    }
    ...
    Context 'Docker Markdown formatter' {
        ...
        It 'Renders curl results in Docker Markdown output' {
            $Data = @{
                Images      = @()
                Containers  = @()
                Dockerfile  = @()
                ...
                CurlResults = @(@{
                    ...
                    Success         = $true
                    Content         = '<html><body>volumetest</body></html>'
            ...
            $Result | Should -Match '## Curl Tests \(1\)'
            $Result | Should -Match 'http://192.168.1.2:8082'
            $Result | Should -Match 'volumetest'
        }

        It 'Renders file contents in Docker Markdown output' {
            $Data = @{
                Images       = @()
                Containers   = @()
                Dockerfile   = @()
                ...
                FileContents = @(@{
                    Path    = '/home/student/volumetest/index.html'
                    Content = "<html><body>volumetest page</body></html>"
                })
            }
            $Result = Format-CollectorDataMarkdown -CollectorResult @{
                Available = $true; Data = $Data; Errors = @()
            } -CollectorName 'Docker' -CategoryName 'Docker' -TargetName 'Linux'
            $Result | Should -Match '## File: /home/student/volumetest/index.html'
            $Result | Should -Match 'volumetest page'
        }

        It 'Renders empty file content as italic message' {
            $Data = @{
                Images       = @()
                Containers   = @()
                Dockerfile   = @()
            ...
            $Result | Should -Match '## File: /home/student/volumetest/index.html'
            $Result | Should -Match '_File is empty or not found._'
        }
    }
