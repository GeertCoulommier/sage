...
## Cross-Platform Compatibility (OSX/Linux/Win)

Enforce strict PowerShell 7.5 compatibility across all operating systems.

- **Paths & Casing**: Use `Join-Path`. Never hardcode `\` or `/`. Treat filenames as case-sensitive.
- **Environment**: Use `[Environment]::UserName`/`MachineName`. Do not use Win-specific `$env:` variables.
- **Cmdlets**: Ban WMI, CIM, Registry (`HKLM:`), and Win32 APIs.
- **OS Checks**: Wrap platform-specific code in `if ($IsWindows)`, `if ($IsLinux)`, or `if ($IsMacOS)`.
- **Files**: Force UTF-8 (no BOM) encoding and LF line endings.
...
- **Quick hook mode:** After verifying tests and/or lint pass, set `$env:SAGE_QUICK_HOOK = '1'` before committing.
- **Always use micro-commits:** one commit per functional change. Each commit must be independently understandable, revertable, and contain exactly one logical unit of work.
- Conventional Commits (`feat:`, `fix:`, `refactor:`, `test:`, etc.).

## AI Collaboration Heuristics

- **Surgical Modification:** Touch only requested lines; do not "improve" adjacent code.
- **Anti-Bloat:** Prefer simple, procedural fixes over complex abstractions; no future-proofing.
- **Deduplication:** Grep the codebase first; re-use existing utilities instead of duplicating logic.
- **TDD:** Add/update Pester tests if needed.
- **Running Pester tests:** ALWAYS use `tools/Run-Tests.ps1` when testing. When working on a specific item or test, test only that section using the parameter `-Filter` with `Run-Tests.ps1`.
- **Terminal output discipline:** Never run commands that produce unbounded output. Pipe through `Select-Object -First`, `head`, or redirect to a file. For E2E runs or long-running processes, use `Get-Process` or log tailing.
