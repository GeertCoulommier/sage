...
## [Unreleased] - 2026-05-31 05:00

### Added

- `Close-RemoteSession`: new `-KeepTempFiles` switch that skips remote temp file cleanup (unlocks P3 lab-mode usage).
- `Invoke-StudentEvaluation`: new `-KeepTempFiles` switch passed through to `Close-RemoteSession`.

### Fixed

- `Invoke-Evaluation`: two `KeepTempFiles` lines were orphaned outside the function body (dead code); moved into both the parallel and sequential `$StuParams` blocks so `Exam.KeepTempFiles = $true` is now correctly forwarded to every `Invoke-StudentEvaluation` call.
...
- **`New-RemoteSession` — variable alignment normalised** — removed extra
  whitespace padding from local variable assignments and fixed the indentation
  of the `while` retry loop inside the `try/finally` block.  No behaviour
  change. *(2026-05-30)*

### Added

- **`devcontainer-lock.json` added** — records the resolved SHA digests for
  devcontainer features so rebuilds are reproducible. *(2026-05-30)*

- **`sshpass` added to devcontainer** — `postCreateCommand` now installs `sshpass` via apt
  so SSH password auth tooling is available inside the devcontainer. *(2026-05-30)*
- **SSH_ASKPASS password auth in `New-RemoteSession`** — when `-Credential` or `-Password`
  (SecureString) is supplied without `-KeyFilePath`, the function now feeds the password to
  the system `ssh` binary via `SSH_ASKPASS` + `SSH_ASKPASS_REQUIRE=force`. A temp shell
  script that echoes a uniquely-named env var (never the password itself) is created and
  cleaned up in a `finally` block. Key-based auth is unaffected. Removes two stale
  `PSReviewUnusedParameter` suppressions that masked this gap. *(2026-05-30)*

### Fixed

- **`Resolve-AdGroupName` — `D` prefix and `-RW`/`-Modify` suffix support** —
  Both `Ad.Tests.ps1` and `FileServer.Tests.ps1` now recognise `D` (short Domain
  Local prefix, e.g. `D_Opleidingen_RW`) alongside the existing `DL`, `DLG`, and
  `L` prefixes.  Write-permission suffix variants `-RW` and `-Modify` are now
  normalised to `-W` so that `D_Opleidingen_RW` resolves to `Opleidingen-W` and
  correctly matches the canonical name used in exam definitions.  Fixes test 2
  ("Group Opleidingen-W or naming variant should exist") and test 8 ("Group HR
  or naming variant should be a member of group Opleidingen-W") for students who
  create Domain Local groups with the short `D_` prefix or with `-RW`/`-Modify`
  write-permission suffixes. *(2026-05-30)*

- **`EmailField` corrected in proefexamen roster config** — `ServerOS-proefexamen.psd1`
  and `Sage/data/exams/proefexamen-2526/exam.psd1` now specify `EmailField = 'StudentEmail'`
  to match the actual CSV column name. The previous value `'email'` caused `$StudentEmail`
  to resolve to `$null`, which surfaced as a cryptic `ValidateNotNullOrEmpty` error deep in
  the grading pipeline rather than an early, clear warning. *(2026-05-29)*
- **Null guard extended to `$StudentEmail` in `Invoke-StudentEvaluation`** — The early-exit
  check that previously only guarded `$StudentIp` and `$StudentName` now also validates
  `$StudentEmail`. A missing email field now emits an immediate `[Pipeline] Warning` and
  skips the student cleanly, instead of propagating a null value into downstream validators.
  *(2026-05-29)*

  `ServerOS-proefexamen.psd1` now uses `LinuxStudentUser`, `WindowsAdminUser`,
  `WindowsStudentUser`, and `DefaultCredential` as vault lookup keys instead of
  the literal password string `'Student1'`, preventing the fallback `Get-Credential`
  prompt from displaying a password as if it were a username. *(2026-05-29)*
- **Vault credential resolution skipped when `KeyFilePath` is provided** —
  `Invoke-Evaluation` no longer calls `Import-Credential` when `-KeyFilePath` is
  supplied. SSH key auth does not use a credential object; the previous code
  triggered a vault lookup (and a fallback prompt) unnecessarily. An `Info` log
  entry is written instead. *(2026-05-29)*

### Added

- **Vault setup instructions in `Import-Credential` help** — The `.NOTES` section
  now documents the three one-time setup steps (install modules, call `Set-Credential`)
  needed before `Import-Credential` can retrieve credentials silently. *(2026-05-29)*

- **SSH post-quantum warning suppressed** — `New-RemoteSession` now sets
  `LogLevel = 'ERROR'` in the default SSH options. This silences the OpenSSH
  9.9+ informational warning *"connection is not using a post-quantum key
  exchange algorithm"* that cluttered TUI output. In a student-lab context the
  warning is not a meaningful security concern; real SSH errors are still
  surfaced. *(2026-05-27)*
- **Docker Compose — container volume mount test (test 4)** — The Docker
  collector now enriches each container with `VolumeMounts` from `docker inspect`,
  providing both `Source` (host path) and `Destination` (container path). The
  evaluator uses this data so the container path check no longer fails. The exam
  definition for category *Docker Compose Linux* now accepts both
  `/home/student/volumetest` and `/home/student/volumetest/html` as valid host
  paths via `AllowedHostPaths`. *(2026-05-27)*
- **Docker Compose — file content test (test 7)** — The Docker collector now
  supports `AllowedPaths` in `FileContentTests`: it tries each path in order
  and stores the content under the primary `Path` key so the evaluator always
  finds it. The exam's `FileContentTests` for *Docker Compose Linux* now lists
  both `/home/student/volumetest/index.html` and
  `/home/student/volumetest/html/index.html` as valid lookup paths. *(2026-05-27)*

### Added

- **Exam definition — ServerOS proefexamen 2025-26** — Added
  `Sage/data/werkcolleges/ServerOS-proefexamen.psd1`, the full exam definition
  for the 2025-26 ServerOS mock exam (300 raw points → /20, 150 Windows + 150
  Linux). Covers General Config, DNS, AD, File Server, GPO, DHCP, IIS, Nginx,
  and Docker categories across DC1, Client, and Linux targets. *(2026-05-27)*
- **TUI config — ExamDefinitionPath updated to proefexamen** — `tui-config.psd1`
  now points `ExamDefinitionPath` to
  `../data/werkcolleges/ServerOS-proefexamen.psd1`. Minor spacing fixes in
  `TargetOrder` and `SelectedTargets` arrays. *(2026-05-27)*

- **Docker collector — Dockerfile instruction parsing** — Each Dockerfile
  instruction (`FROM`, `COPY`, `RUN`, `EXPOSE`, etc.) is now stored as a
  separate key in the collected object (multi-value instructions become arrays).
  Case-sensitive `-cmatch` and `$InContinuation` tracking prevent false matches
  on lowercase continuation lines (e.g. `chmod`, `CMD` inside `HEALTHCHECK`).
  *(2026-05-27)*
- **Docker collector — compose file YAML parsing** — Compose files are now
  parsed into structured objects using a self-contained PowerShell parser:
  top-level sections (`services`, `networks`, `volumes`) and service
  properties (`image`, `container_name`, `restart`) are accessible as
  hashtable keys. No external dependencies required. *(2026-05-27)*
- **Docker collector — compose list fields parsed as arrays** — Service
  properties that are YAML lists (`ports:`, `volumes:`, etc.) are now captured
  as PowerShell arrays.  Previously, only scalar key:value pairs at indent-4
  were stored; list items at indent-6 were silently dropped.  The parser now
  tracks the current list key and appends each `- item` entry to the array.
  *(2026-05-27)*
- **Docker evaluator — accept all compose filename variants** — `ComposeTests`
  and `ComposeContentTests` now match compose files by directory + any of the
  four accepted filenames (`compose.yml`, `compose.yaml`, `docker-compose.yml`,
  `docker-compose.yaml`, case-insensitive).  Students may use any of these
  filenames without penalty. *(2026-05-27)*
- **Docker evaluator — ComposeContentTests use structured data** — Tests in
  `ComposeContentTests` now use `Key` + `ContainsValue` instead of
  `ContainsLines`.  The evaluator checks the parsed service data for the named
  key and verifies the value (scalar or array element) matches the expected
  string.  Removed dependency on raw YAML text format. *(2026-05-27)*
- **TUI Docker formatter — CurlResults and FileContents sections** — Both the
  plain-text (`Format-DockerData`) and Markdown (`Format-DockerDataMarkdown`)
  formatters now render live-curl test output and collected file contents.
  Plain-text shows `[OK/FAIL] <URL>` and file previews; Markdown wraps responses
  in `html` code fences with success symbols. *(2026-05-27)*

- **TUI Settings — Remove tui-config-personal.psd1 action** — New menu item at
  the bottom of Settings that deletes `tui-config-personal.psd1` after
  confirmation, allowing a clean reset of all saved TUI preferences. *(2026-05-24)*
- **TUI Settings — Remove SSH key pair action** — New menu item that deletes
  `tui/keys/id_sage` and `id_sage.pub` after confirmation, so the SAGE key
  pair can be regenerated on the next connection setup. *(2026-05-24)*

### Fixed

- **Docker collector — Dockerfile paths extracted from DockerfileTests variables** —
  `Invoke-DockerCollector.ps1` now seeds `$DockerfilePaths` directly from
  `$Variables.DockerfileTests[].Path` before falling back to the recursive
  `Get-ChildItem` search. This ensures the collector reads the Dockerfile even
  when the filesystem search returns nothing (e.g., due to path permissions or
  PowerShell quirks on the remote Linux VM). *(2026-05-27)*
- **Docker collector — Null array initialization bug fixed (Dockerfile + Compose)** —
  `$DockerfilePaths` and `$ComposePaths` were initialized via
  `$x = if (...) { ... } else { @() }`. In PowerShell, an empty array `@()` passed
  through the pipeline in an `if`-expression context produces zero items, so
  `$x` was assigned `$null` instead of `@()`. Subsequent `$x += path` calls then
  did string concatenation rather than array append — all paths were smashed into
  a single concatenated string that `Test-Path` never found. Fixed by initializing
  explicitly: `$x = @()` followed by a separate `if` block. *(2026-05-27)*
- **Docker collector — Compose paths extracted from ComposeTests/ComposeContentTests** —
  Same pattern applied to compose files: paths from `$Variables.ComposeTests` and
  `$Variables.ComposeContentTests` are now seeded before the recursive search. *(2026-05-27)*
- **Docker collector — Curl output stored as single joined string** —
  `curl` command output was stored as a `string[]` (one element per line).
  The evaluator test `$MatchingResult.Content | Should -Match '...'` piped each
  line individually; lines that didn't contain the expected text caused the test
  to fail despite the content being present. Fixed by joining the output with
  `` `n `` before storage, consistent with how Dockerfile and Compose contents are
  stored. *(2026-05-27)*

- **Docker Images Linux — proefexamen-2526 test values corrected** —
  `exam.psd1` C10 section: `ExpectedFrom` corrected to `'httpd'` (prefix
  match accepts any `httpd:*` tag); `ContainerPort` relaxed from `'80/tcp'`
  to `'80'` (matches `80/tcp`, `80/udp`, etc.); `ExpectedCopy` changed from
  `'./index.html …'` to `'index.html …'` so all valid source paths are
  accepted; `ExpectedContent` in `CurlTests` corrected from `'httpd_docker'`
  to `'apache in docker'` to match the actual HTTP response. *(2026-05-27)*
- **Docker evaluator — tests updated to use structured fields** —
  `Docker.Tests.ps1` FROM and COPY Dockerfile tests now read `$MatchingFile.FROM`
  and `$MatchingFile.COPY` (structured instruction keys) instead of running
  regex against raw `Content`. Compose services test reads `$MatchingFile.services.Keys`
  from the parsed YAML hashtable. *(2026-05-27)*
- **Docker exam — compose file path and service name corrected** —
  `exam.psd1` C11 section: compose path changed from `docker-compose.yml` to
  `compose.yml` (actual filename on the VM); expected services corrected from
  `@('volumetest', 'nginx')` to `@('nginx')` (`volumetest` is the
  `container_name`, not the service name). *(2026-05-27)*

### Changed

- **Collector formatter alignment normalization** —
  `Format-CollectorData.ps1` alignment and quoting were normalized for
  readability and consistency (no behavior changes). *(2026-05-24)*

- **Collector data pipeline documentation** — Added
  `docs/internal/COLLECTOR-DATA-PIPELINE.md` documenting collector data flow
  from remote collection through grading, export, and TUI/Edit-Grade display.
  *(2026-05-24)*

- **IIS collector — DirectoryBrowsing reads from web.config via GetWebConfiguration** —
  `GetApplicationHostConfiguration().GetSection()` only reads `applicationHost.config`
  and misses settings written to per-directory `web.config` files (the default when
  a student enables Directory Browsing via IIS Manager). Fixed by additionally calling
  `$Mgr.GetWebConfiguration(siteName, virtualPath).GetSection(...)` and OR-ing both
  sources: a vdir reports `DirectoryBrowsing = true` when either `applicationHost.config`
  (location tag) or the vdir's own `web.config` has `enabled = true`. Applied to both
  the PS 7 direct path and the PS 5.1 fallback scriptblock.
  Three new unit tests cover: web.config true, applicationHost true, both false.
  *(2026-05-24)*

- **IIS display order — Curl Tests → App Pools → Websites** — `Format-IisData`
  and `Format-IisDataMarkdown` now render sections in the order: HTTP/Curl
  Tests first, App Pools second, Websites last. Within the websites list
  `Default Web Site` is always sorted to the bottom. *(2026-05-24)*

- **IIS collector — auth reads per-vdir via location path** — `Invoke-IisCollector`
  previously read authentication settings using `$App.GetChildElement()`, which
  only sees the application-level web.config context and misses virtual-directory
  overrides stored as `<location>` tags in `applicationHost.config`. Fixed by
  calling `$Mgr.GetApplicationHostConfiguration().GetSection(sectionPath, locationPath)`
  with the correct per-vdir location path (e.g. `proef.be/vDir1`). Directory
  browsing is likewise now read per-vdir from the location config. Both the PS 7
  direct path and the PS 5.1 fallback path are updated. `IndexPath` added to site
  data. *(2026-05-24)*
- **IIS display — auth, dir browsing, curl results, index file** —
  `Format-IisData` and `Format-IisDataMarkdown` now show per-vdir authentication
  state (enabled methods listed), directory-browsing status, HTTP/curl test
  results with response preview, and the index file path and content for each
  site. *(2026-05-24)*
- **DNS PTR record allows linux/rockylinux** — PTR record `2` in the
  `1.168.192.in-addr.arpa` reverse zone now accepts both `linux.proef.be` and
  `rockylinux.proef.be` via `AnyOfExpectedPtr`. DNS evaluator updated with a
  new `It` block to handle multi-allowed-name PTR tests. *(2026-05-24)*
- **GPO link tests split** — The `X-drive-mapping-Opleidingen` link test is
  now two separate entries: one for `ExpectedLinksAll` (5 pts) and one for
  `ForbiddenLinks` (2 pts), keeping the total at 7 points. *(2026-05-24)*
- **GPO drive mapping regex** — `ExpectedPathRegex` for
  `X-drive-mapping-Opleidingen` updated to accept both
  `\\dc1.proef.local\Shared\Opleidingen` and `\\dc1\Shared\Opleidingen`,
  with optional trailing backslash, case-insensitive. *(2026-05-24)*
- **GPO link format** — `ExpectedLinksAll` and `ForbiddenLinks` entries
  changed from LDAP format (`OU=Marketing,DC=proef,DC=local`) to short OU
  names (`Marketing`), matching the `SOMPath` format returned by the
  collector (`proef.local/Marketing`). *(2026-05-24)*
- **GPO link test labels** — Each `GpoLinkTests` entry now carries a
  `LinkExpectation` field; the Pester `It` template reads it so labels
  correctly say "should be linked to …" or "should NOT be linked to …".
  Drive-map label now shows the expected UNC path via the new `DrivePath`
  field. *(2026-05-24)*
- **GPO drive letter matching** — Drive-letter comparison in `Gpo.Tests.ps1`
  now strips any trailing colon so `X` and `X:` both match the collector
  value `"Letter": "X:"`. *(2026-05-24)*
- **GeneralConfig Markdown formatter — network adapter data** — Gateway, DNS
  servers, subnet mask, and DHCP/Static assignment type are now correctly
  resolved from `NetAdapters` (the real collector output structure) and
  rendered in the drill-down collector panel. *(2026-05-24)*
- **TUI Settings — menu order** — Items are now shown in a logical order:
  Domain Name, Targets, Output directory, Edit tui-config-personal.psd1,
  Theme, then destructive actions (Clear results, Remove config, Remove SSH
  keys) at the bottom. *(2026-05-24)*
- **TUI Settings — Edit config action targets personal file** — "Edit
  tui-config-personal.psd1" now opens the personal config file (copying it
  from the vanilla `tui-config.psd1` first if it does not yet exist) instead
  of the read-only vanilla config. *(2026-05-24)*

### Fixed

- **`tui-config.psd1` — OutputDir path typo** — Corrected `'.../data/output'`
  to `'../data/output'` in the default vanilla TUI config. *(2026-05-24)*

### Refactored

- **`Invoke-LocalEvaluation` — variable alignment** — Removed unnecessary
  padding alignment from local variable assignments for consistency with the
  rest of the codebase. *(2026-05-24)*

### Fixed

- **Category ordering in TUI results** — `Invoke-LocalEvaluation` now
  reorders `CategoryScores` after merge to match the exam definition order,
  so categories for each device are shown in exam order instead of
  alphabetically. *(2026-05-24)*
- **`Format-GeneralConfigData` — subnet mask, assignment type, gateway/DNS** —
  Updated the general-config formatter to: show subnet mask (computed from
  `PrefixLength`); show assignment type (`Static`/`DHCP`) from `PrefixOrigin`;
  look up gateway and DNS servers from the `NetAdapters` array (linked by
  `InterfaceAlias`) rather than the `IPAddresses` array where those fields are
  absent. Updated unit test data to match the real collector data structure.
  *(2026-05-24)*
- **DNS PTR record check — first hostname label only** — Changed the PTR record
  matcher in `Dns.Tests.ps1` to only require the first DNS label (e.g. `dc1`)
  of `ExpectedPtr` to match, allowing the domain suffix to vary between
  environments (e.g. `dc1.proef.be` or `dc1.proef.local`). *(2026-05-24)*
- **AD: user OU placement validation** — Added `ExpectedOU` support to
  `Ad.Tests.ps1` `UserTests`; added `ExpectedOU = 'OU=Pers,OU=Marketing'` for
  `Daan.Banaan` in proefexamen-2526. *(2026-05-24)*
- **AD formatter — groups before users, split and table format** — Rewrote
  `Format-AdData` to render user-created groups (DN contains `OU=`) and
  default/system groups as separate autosized tables (Name | Scope | Members
  count), with member DNs indented below each group. Users section moved to
  the end. *(2026-05-24)*
- **FileServer formatter — user vs system shares** — Rewrote
  `Format-FileServerData` to display user-created shares first (`User Shares`)
  and well-known system shares (`NETLOGON`, `SYSVOL`, `IPC$`, `ADMIN$`, any
  ending with `$`) in a separate `System Shares` section. NTFS permissions are
  now shown inline per share. *(2026-05-24)*
- **AD groups: member OU validation** — Added `RequiredMemberOU` support to
  `GroupHasMembersTests` in `Ad.Tests.ps1`; updated proefexamen-2526 HR group
  to require a member in `OU=HR` and Pers group to require a member in
  `OU=Pers`. *(2026-05-24)*
- **C6-bg NTFS check — Public-R group** — Updated proefexamen-2526 C6-bg
  `NtfsTests` to check for `Public-R` (or `Public-Read`/`Public-Lezen`) with
  `ReadAndExecute` on `Shared\Public`, replacing the incorrect `Opleidingen-R`
  check. Added `Public-R` (DomainLocal Security) to C5b `GroupExistenceTests`
  and `Domain Users` → `Public-R` membership to `GroupInGroupTests`.
  *(2026-05-24)*
- **exam.psd1 — credential secret and Pers OU** — Changed
  `DefaultCredentialSecret` from `StudentPassword` to `Student1`; corrected
  Pers OU `ExpectedDN` to `OU=Pers,OU=Marketing,DC=proef,DC=local`.
  *(2026-05-24)*
- **tui-config — proefexamen-2526 path and target order** — Updated
  `ExamDefinitionPath` to point to the proefexamen-2526 exam; set
  `TargetOrder` to `DC1, Client, Linux`. *(2026-05-24)*

### Fixed

- **`Test-ExamDefinition` — allow `PassGrade = 0`** — The validator previously
  rejected `PassGrade = 0` with "must be > 0", blocking informational/zero-weight
  test entries that are intentional in exam definitions (e.g. `Opleidingen-FC`
  existence check, `Pers` membership check, `DefaultAppPool` IIS check, Docker
  Compose structural check). Changed validation from `-le 0` to `-lt 0` so only
  negative values are rejected. Updated the corresponding Pester test to assert
  that `PassGrade = 0` is now allowed. *(2026-05-23)*
